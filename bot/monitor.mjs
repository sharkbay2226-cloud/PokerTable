...
async function issueLicenseForOrder(order, bot, txValue) {
  ...
  try {
    ...
    const msg = [
      ...
      `💵 <b>Сумма:</b> $${txValue || order.amount_usdt} USDT`,
  ...
  💰 <b>Заказ #${order.id}</b>
  👤 ${name} | @${order.username || '—'}
  💎 ${PLAN_NAMES[plan]} | $${txValue || order.amount_usdt}
  🔑 <code>${licenseKey}</code>
...
async function retryFailedLicenses(bot) {
  const failed = rows(
    "SELECT o.*, u.username, u.first_name FROM orders o LEFT JOIN users u ON o.user_id=u.id WHERE o.status='confirmed' AND o.license_key IS NULL"
  );
  for (const order of failed) {
    console.log(`[${ts()}] Retrying license for confirmed order #${order.id} (user ${order.user_id})`);
    const ok = await issueLicenseForOrder(order, bot, order.amount_usdt);
    if (ok) {
      console.log(`[${ts()}] License issued for order #${order.id}`);
    }
  }
}
...
async function processPendingOrders(bot) {
  ...
  try {
    ...
    transfers.sort((a, b) => a.created_at < b.created_at ? 1 : -1);
    for (const tx of transfers) {
      ...
      await issueLicenseForOrder(order, bot, tx.value);
    }
  } catch (e) {
    console.error(`[${ts()}] processPendingOrders error:`, e.message);
  } finally {
    running = false;
  }
}
...
async function main() {
  ...
  const tick = () => {
    processPendingOrders(bot).finally(() => setTimeout(tick, POLL_INTERVAL));
  };
  tick();
}
